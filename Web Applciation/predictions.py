...
def getPredictions(image):
    ...
    for l, r, t, b, label, token in img_tagging.values:
        ...
        # Draw the detected label or token safely
        cv2.putText(img_bb, str(label[0]) if isinstance(label, np.ndarray) else str(label),
                (l, max(t - 5, 15)), cv2.FONT_HERSHEY_PLAIN, 1, (255, 0, 255), 2)
